...
function findFirstMissingNumber(array) {
  ...
  while (difference !== 1) {
    difference = array[lastSequentialIndex + 1] - (array[lastSequentialIndex] + 1) - nums;
    nums++;
  }
...
function isHoliday(date) {
  return !!holidays.find((dateString) => {
    ...
    return date.toString() === new Date(dateString).toString();
  });
}
...
const numbers1 = [-1, -100, -5, 10, 0, 11];
