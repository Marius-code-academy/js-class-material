...
function nameLengths(arr) {
  ...
  const lengths = arr.map((name, i) => name.length);
...
function convertToFahrenheit(arr) {
  const tempFar = arr.map((temp) => (temp * 9) / 5 + 32).filter((temp) => temp > 40);
  console.log(tempFar);
}
...
function getEvenNumbers(arr) {
  ...
  const filtered = arr.filter((num) => num % 2 === 0 && num > 3);
...
function findFirstLongWord(arr) {
  const longWord = arr.find((word) => word.length > 5);
...
const stringsArray = ["apple", "banana", "grapefruita", "orange"];
...
function areAllAdults(arr) {
  ...
  const isAllAdult = arr.every((age) => age > 18);
...
const wordsArray2 = ["ppl", "banana", "grape", "kiwi"];
